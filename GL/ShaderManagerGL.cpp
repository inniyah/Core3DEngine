...
namespace Core {
    ...
    ShaderManagerGL::ShaderManagerGL() {
        ...
        this->ParticleStandard_vertex = 
            ...
            "in float rotation;\n"
            "in vec2 size;\n"
            "in vec4 sequenceElement;\n"
            "in vec4 color;\n"
            "out vec4 vViewPosition;\n"
            "out float vRotation;\n"
            "out vec2 vSize;\n"
            "out float vSequenceElement;\n"
            ...
            "   vRotation = rotation;\n"
            "   vSize = size;\n"
            "   vSequenceElement = sequenceElement.x;\n"
            "   vColor = color; \n"
        ...
        this->ParticleStandard_geometry =
         ...
         /*  "void getUV(in int sequenceElement, in int sequenceNumber, in vec4 atlasFrameSet, out vec2 uv) { \n"
            ...
            "   vec4 color = vColor[0];\n"
            "   float sequenceElement = vSequenceElement[0];\n"
            ...
            "   mat2 rotMat = mat2(cos(rotation), -sin(rotation), sin(rotation), cos(rotation)) * mat2(particleSize.x, 0.0, 0.0, particleSize.y);\n"
            "   gl_Position = " + PROJECTION_MATRIX + " * (vec4(rotMat * dLeft, 0.0, 0.0) + vViewPosition[0]);\n"
            "   vUV1 = vec2(uv1.x, uv1.y);\n"
            "   vUV2 = vec2(uv2.x, uv2.y);\n"
            "   vFragColor = vec4(color); \n"
            "   EmitVertex();\n"
            "   gl_Position = " + PROJECTION_MATRIX + " * (vec4(rotMat * uLeft, 0.0, 0.0) + vViewPosition[0]);\n"
            "   vUV1 = vec2(uv1.x, uv1.y + atlasTileHeight);\n"
            "   vUV2 = vec2(uv2.x, uv2.y + atlasTileHeight);\n"
            "   vFragColor = vec4(color); \n"
            "   EmitVertex();\n"
            "   gl_Position = " + PROJECTION_MATRIX + " * (vec4(rotMat * dRight, 0.0, 0.0) + vViewPosition[0]);\n"
            "   vUV1 = vec2(uv1.x + atlasTileWidth, uv1.y);\n"
            "   vUV2 = vec2(uv2.x + atlasTileWidth, uv2.y);\n"
            "   vFragColor = vec4(color); \n"
            "   EmitVertex();\n"
            "   gl_Position = " + PROJECTION_MATRIX + " * (vec4(rotMat * uRight, 0.0, 0.0) + vViewPosition[0]);\n"
        ...
        this->Lighting_fragment =
           ...
           /* "    for (int i = 0 ; i < " + LIGHT_CASCADE_COUNT + "[@lightIndex]; i++) { \n"
            ...
            "                if (length(worldPos.xyz - " + LIGHT_POSITION + "[@lightIndex].xyz) >= " + LIGHT_RANGE + "[@lightIndex]) { \n"
